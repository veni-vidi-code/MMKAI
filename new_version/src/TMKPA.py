...
class TMKPA:
    ...
    def _adjust_matching(self, matching_save: MatchingSave, remaining_capacity: list[int]):
        ...
        for node in list(graph.nodes):
            if isinstance(node, tuple) and node[1] > remaining_capacity[node[0]] // item_class.weight:
                graph.remove_node(node)
                if node in matching:
                    x = matching.pop(node)
                    if match_vec[node[0]] < remaining_capacity[node[0]] // item_class.weight:
                        match_vec[node[0]] -= 1
                        matching.pop(x)
                    else:
    ...
    def _lower_bound(self, current_fixed_itemclass: int, current_fixed_knapsack: int, fixed_to: int,
        ...
        remaining_capacity = current_save.remaining_capacity.copy()
        ...
        bound = sum(current_save.match_vec[current_fixed_knapsack:]) * self.item_classes[
    ...
    def _bound(self, current_itemclass: int, current_knapsack: int, fixed_to: int,
        ...
        if L > self.best_solution_value:
            ...
            self.best_solution = heuristic_solution.copy()
    ...
    def solve(self) -> (int, dict[Knapsack, set[Item]]):
        if not self.solved:
            ...
            self.best_solution_value = L
            self.best_solution = matchings.copy()
